...
**CASE-BATCH-5000-20260926（源码及专项验证完成，未生产加载）**：上限改为 5000，预览明确跳过已关闭及证据身份错连案件。追加后的 82 项批量回归通过，覆盖完整 5000 案×5 条证据、三类混选、全异常、异常原件不变、确认后新增异常拒绝、重复确认、超限和末尾故障全回滚；Ruff/格式/mypy 通过。完整回归与精确 HEAD CI 的最终收据入口见 [批量记录](docs/2026-09-23-case-batch.md#case-batch-5000-20260926批量人工处理扩大到-5000-案)，不能沿用初轮编码不一致的失败结果。生产只读盘点，未处理真实案件。负责人反馈全页 CSV 导出和其核对的客户端撤回成功，作为真人观察记录，不回填后台通知计数。
