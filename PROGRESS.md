...
测试数字绑定环境：本机全量收集 **1106 / 1093 passed / 13 条件跳过 / 0 失败**；主审 R-111 环境 1098 passed / 1 跳过（收集 1099）；CI（`4c57b6c`）1096 passed / 3 skipped——三数互不替代。ruff/format/mypy 85 文件通过；新 SHA CI 以 PR #7 检查页为准。**Windows 未部署本轮修复**（待主审复验后决定），真实动作保持关闭；恢复动作/生产删除/历史重写未授权。现场待办：真实 TUN 下载实测、隔离副本清理/恢复验收、独立新 W2、百群负载与延迟、通知接管/故障演练、W3–W5。
