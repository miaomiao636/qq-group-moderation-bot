...
负责人已选择：正常访问恢复后验证低频分批。桌面现为每批最多 10 人、批内间隔 30 秒、批末自动停止且手动续批；这是实验参数，不是腾讯安全频率。尚未进行恢复后的真实批次验证。 具体代码、执行 SHA/命令与 GitHub 源码研究结论见 [巡检记录](docs/2026-09-21-space-inspector.md)。
