...
**2026-09-15（R-111 / v13.1）：修复提交 `feb821c` 已推送**（SHA 三口径：功能提交 `24d1202` / 受审 head `4c57b6c` / 修复 `feb821c`）。主审 R-111 对 `4c57b6c`：B-2 主要行为通过（16 端到端场景）、三平台 CI 全绿、无新 P0/P1；1 个 P2（转人工时 ad 类别覆盖严重疑似类别）本轮已修：严重疑似优先选类别（确定性）+ 类别置信度同源，verdict 恒 record_only、只改标记不改处罚；反馈表单类别改可核对/纠正下拉（白名单校验）；办证 record_only 底线固化。新增 7 项回归。

测试数字绑定环境：本机全量收集 **1106 / 1093 passed / 13 条件跳过 / 0 失败**；主审 R-111 环境 1098 passed / 1 跳过（收集 1099）；CI（`4c57b6c`）1096 passed / 3 skipped——三数互不替代。ruff/format/mypy 85 文件通过；**新 SHA CI 已核验全绿**（`feb821c` run 34877755494、`4f7e90d` run 34877850499，三 job success）；PR #7 正文已更新"最新核验入口"节。**Windows 未部署本轮修复**（待主审复验后决定），真实动作保持关闭；恢复动作/生产删除/历史重写未授权。现场待办：真实 TUN 下载实测、隔离副本清理/恢复验收、独立新 W2、百群负载与延迟、通知接管/故障演练、W3–W5。
