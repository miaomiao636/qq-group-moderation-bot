...
**主通道架构调整为NapCat/OneBot（2026-09-07，决策D-019）**：项目负责人已实际确认，个人认证官方机器人无法开启“添加到任意群聊”，在数百/数千人目标群的邀请列表中不显示。因此QQ官方机器人降为可选/测试通道，NapCatQQ + OneBot 11改为生产大群主通道。本轮架构调整基于代码基线 `88ac433`；该基线包含官方通道契约、审核、AI、动态规则、案件、报告和官方动作编排，**尚无NapCat入站与撤回/禁言/警告Adapter**。当前必须先完成R-104、T-305、T-306、T-303和T-307，才能进入NapCat目标群分阶段上线。

**当前质量阻塞**：`main` @ `88ac433` 的GitHub Actions运行 `34083954491` 在Ubuntu和Windows均因 `app/reports/stats.py` 未通过 `ruff format --check` 失败，后续mypy和pytest被跳过；干净运行时依赖任务成功。R-104尚未整改，不能声称当前 `main` 跨平台CI全绿。
