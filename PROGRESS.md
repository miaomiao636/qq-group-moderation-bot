...
**R-102交付后独立复验未通过（2026-09-06，main `ce2f2f7`）**：现有160项测试、mypy和ruff通过，但已独立复现并发重复领取、永久解析失败重试唯一键冲突、媒体配额越界/文件名碰撞，以及反诈/否定/咨询文本被硬黑名单自动判高风险；R006也与允许来源群规冲突。R-103整改与动态规则/AI/反馈学习设计已固化，代码尚未开始修改。**真实撤回/禁言/警告/NapCat仍未启用**。

## 已完成

- **R-103及AI规则学习设计与独立复验（2026-09-06）**：新增 `docs/superpowers/specs/2026-09-06-ai-rule-learning-design.md`；将工作拆为R-103、T-105、T-204、T-205、T-106。独立执行现有全量门禁：160项pytest通过（1项skip）、mypy 41文件通过、ruff check/format通过；额外故障脚本复现的结果记录在“已知问题”。本项只完成设计与取证，未声称缺陷已修复。
...
- **事件并发领取（R-103阻塞）**：两个独立数据库会话对同一新消息连续调用 `begin_processing`，实测结果为 `True, True`；未过期 `PROCESSING` 被立即视为可重试，存在重复处理风险。
- **永久解析失败重试（R-103阻塞）**：缺少 `group_openid` 的同一事件第一次生成 `ShadowDecision`，第二次投递实测触发 `UNIQUE constraint failed: shadow_decisions.message_id`。
- **媒体存储（R-103阻塞）**：目录已有8字节、配额10字节时仍接受5字节下载，最终13字节；只保留消息ID尾24字符导致不同ID生成同名文件；魔数嗅探调用 `read_bytes()` 会先读取整个文件。
- **规则误伤（R-103阻塞）**：`大家觉得兼职靠谱吗？我怕被骗`、`学校让我们讨论刷单骗局，请大家不要上当`、`我不做兼职，谢谢` 均实测为 `violation_high`；所有分享卡片直接R006高风险，与 `docs/group-rules.md` 的允许来源规则冲突。
- **后台与新能力（R-103/T-105/T-204/T-205）**：规则页只读且未统一鉴权，POST缺少CSRF；规则为硬编码常量；远程AI、人工反馈标签、候选规则回放/发布均尚未实现。
- **运行状态（T-106/T-404）**：运行器仍为影子模式；Windows Service、自启动、重启和更新恢复尚未实现或实机验收。
...
- 尚无NapCat身份镜像和踢人执行器；QQ官方适配、审核、案件、后台和报告已有代码，但未完成正式动作与整体验收。
