...
**CASE-REOFFEND-BATCH-20260925（本地验证完成，生产待加载）**：按 D-042 恢复人工待审案的形成，不恢复自动警告、禁言或踢人。跨页批量预览跳过已关闭重复选项，只对仍待审案创建冻结计划；已人工结案成员在同群再犯另建新案，不重开旧案。案件编号缺口、误判撤销、损坏审计及归档后前案引用均补回归。完整 `uv run --locked pytest -q --tb=line` 退出码 0；Ruff 检查/格式、`mypy app` 与差异检查通过。GitHub CI 和生产加载单独核验。现有自动维护任务负责已关闭满 15 天归档，归档满 90 天后有条件精简，非硬删除。
