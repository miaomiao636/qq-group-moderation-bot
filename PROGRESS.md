...
**CASE-ARCHIVE-30D-20260926（D-043，源码验证完成）**：清理器对尚未归档且 `CLOSED`、结案满 30 天的案件归档；恰好满 30 天的边界纳入，未满/待审不归档，既有归档不反转。后台说明同步改为 30 天。原文独立 15 天清理与归档后 90 天安全精简保持不变。新边界先红后绿，相关清理/页面测试和本机全量 pytest、Ruff、mypy、差异检查退出码均为 0。精确新提交的远端 CI 和维护任务下一次真实执行仍需核验；本批未触发生产清理或重启服务。实时运行状态与交付缺口以本轮交接为准。
