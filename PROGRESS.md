...
**CASE-BATCH-20260923：源码已推送，未部署。** `d3e2d3c` 的案件摘要导出、冻结预览后批量保留结案及撤回回执超时 UNKNOWN 修复已实现并验证；回归入库、完整 SHA/命令、全量/门禁、原主审和 CI 核验入口见 [本轮记录](docs/2026-09-23-case-batch.md)。没有处理线上案件或改动作策略，生产最近验证源码仍为 `23d0a1f`；部署另需本轮重启授权。
