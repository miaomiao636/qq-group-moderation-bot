...
**2026-09-15（R-111 / v13.1 两轮 P2）：最新修复 `76ce10b` 已推送**（SHA 三口径：功能提交 `24d1202` / 受审 head `4c57b6c`+`bd5402c` / 修复链 `feb821c`→`76ce10b`）。首轮：转人工分支严重疑似优先选类别 + 类别置信度同源（只改标记不改处罚）、反馈类别可核对/纠正、办证 record_only 底线固化（7 回归）；主审复验通过该分类修复，指出**二轮 P2**——反馈回显取「最早一条」与学习侧 max(ID) 口径不一致（二次纠正后只改原因再提交会反转真值），**已修（`76ce10b`）**：列表/详情统一按 `FeedbackRecord.id` 倒序取最新 + 9 回归（二次纠正回显、只改原因不回退、同秒/时钟回拨、最新 label/reason、无反馈默认值、缺 CSRF 不写库）；v13 helper 补 source、组 B 显式 vision。
