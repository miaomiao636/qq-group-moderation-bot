...
**RECALL-CONFIRM-20260924：已实现，本地全量验证通过，准备部署。** API 回包与 QQ 撤回通知分开持久化，未确认不升级处罚；只读审查修正了通知时钟来源与详情关联，新迁移/备份/旧库维护兼容同步验证。实际命令、SHA、原探针版本元数据适配及待部署边界见 [本轮记录](docs/2026-09-24-recall-confirmation.md)。生产未重启、未迁移、未重放。

**CASE-SELECT-20260923：跨页选择与人工处理补正已实现，尚未部署。** 实现 `4855a4a`、入库回归 `07ec9ba`；同一标签页/登录/筛选内累计多页勾选，取消理由输入，默认审计“人工处理”，保留预览确认。完整本地测试、门禁与无网络前端脚本模拟已通过；精确执行 SHA/数量、最新 HEAD CI 和生产授权边界见 [案件批量补正](docs/2026-09-23-case-batch.md#case-select-20260923跨页选择与人工处理补正待上线)。生产仍为上一批，未改案件、群开关或处罚配置。
