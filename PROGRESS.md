...
**⚠️ 当前状态与待办**：**数据库级急停处于开启状态**（本次维护窗口按手册第 1 步开启，审计 `emergency_stop`）→ **真实动作（撤回/禁言/警告）当前暂停**，系统仅记录影子判定。待负责人在后台「影子判定」页观察"合并转发/群名片"是否误判后，到「群管理」→「预览解除急停」解除；解除后 D-038 撤回在 13 个授权群按既有配置生效（`recall_only` 阶段只撤回、不禁言）。此前记录的"半部署即时风险"已解除（库与代码版本一致）。D-037 属高风险政策，建议送主审独立复验。
