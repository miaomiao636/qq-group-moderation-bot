...
- 最终全套：**754 passed / 1 skipped / 1个原有sqlite datetime adapter弃用警告**。`mypy app`及`mypy --platform win32 app`均83源文件通过；ruff check/format172文件、diff及离线锁文件检查通过。初次受限环境两项启动测试因无法绑定回环端口受阻，获准本地端口后整套重跑通过，未跳过用例或改断言。临时库迁移升级/回退/再升级及旧数据保持有实测；隔离仅运行时环境导入应用/通知/探针成功且pytest未安装，另有干净锁定安装复验；默认探针CLI实际返回skipped，不产生外呼。
