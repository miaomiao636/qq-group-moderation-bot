...
**COMPANION-READY-20260926（技术交付整改）**：已实现新安装模式、固定清单来源与恢复源码验证、巡检离线数据备份和隔离恢复，并同步公司手册、MIT 元数据与包收据。基线 `0c82cd4`；本轮源码冻结后执行全量、门禁、干净包验证和 CI，结果统一追加在 [交付整改](docs/2026-09-25-companion-delivery.md#companion-ready-20260926)。未操作生产服务、数据库、计划任务、群开关或 QQ，会话与用户真实数据不进入候选包；公司新环境验收未完成。
