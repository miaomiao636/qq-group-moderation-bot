...
**CASE-EXPORT-FIELDS-20260924：源码已修复，待上线。** 跨页勾选原先逐案提交表单字段，超过后台 1000 字段上限时导出在解析阶段失败；现将选中 ID 合并为单字段，保持最多 5000 案、CSRF 与原身份/案件校验。合成 1001 案 multipart CSV 导出、1200 案前端脚本、全量测试与静态检查通过；未导出或更改生产案件，当前服务尚未加载修复。详见 [案件批量记录](docs/2026-09-23-case-batch.md#case-export-fields-20260924跨页导出字段上限修复)。
