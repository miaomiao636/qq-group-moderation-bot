...
**DAILY-BACKUP-20260922（备份与恢复验证完成，定时任务待管理员注册）**：源码 `7b040b8` 已推送并同步本机；排除服务凭据的明文备份、完整隔离恢复、全量/门禁/源码 CI 已通过，执行 SHA、命令和证据见 [每日备份](docs/2026-09-22-daily-backup.md)。Windows 管理员进程未成功启动，QQBotDailyBackup 尚未创建，不能称为每日自动备份已启用。实时急停 false、主服务原进程及群授权/动作保持不变；旧备份和证据保留。
