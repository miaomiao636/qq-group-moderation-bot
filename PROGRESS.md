...
**PROJECT-AUDIT-20260922：全面复查修复已推送，尚未部署**。冻结源码 `1c17ba09e5bef268a6162864830396e8d36c6107` 的全量、原主审、门禁与精确源码 CI 已验证；认证、群备注并发、连接资源、报告快照、备份边界与媒体依据修复及真实运维缺口见 [本轮审查](docs/2026-09-22-project-audit.md)。生产仍是 `06324fe` / `t204-v18`，急停 true；无生产清理、权限、计划任务或通知变更。以下旧批次保留历史，当时的“当前/未部署”不覆盖本条状态。

**CAMPUS-TEMPLATE-20260922 已上线，急停保持开启**：当前生产加载 `06324fedab4baf8d0ee14e09991c5385627a01ed`，提示词 `t204-v18`。认可分享卡按图标、完整页脚和同卡布局联合识别；单图及普通文字配图均可建立同成员两分钟窗口。源码、真实原图复验、离线窗口回放、CI 与两次部署证据见 [本轮记录](docs/2026-09-22-campus-source-scope.md)。未解除急停、未修改群授权或动作开关；新规则不重判历史消息。
