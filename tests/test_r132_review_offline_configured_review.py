...
class FixedVision:
    ...
    async def moderate_image(self, request):
        self.calls += 1
        return AIModerationResult(source='vision', model_id=self.model_id,
                                  category='ad', confidence=self.confidence,
                                  needs_review=False, evidence='synthetic')


@pytest.mark.parametrize('tool', ['replay', 'export'])
@pytest.mark.parametrize('low,high,pconf,sconf,accepted', [
    (.60, .95, .70, .92, False),
    (.50, .90, .55, .95, True),
    (.60, .90, .70, .95, True),
...
async def test_offline_cannot_guess_thresholds_for_actual_persisted_pair(
    ...
    group = str(ident())
    service = AIReviewService(enabled=True, enabled_groups={group},
        vision_moderator=primary, review_vision_moderator=secondary,
        secondary_review_low=low, secondary_review_high=high)
    row = await execute(event(group, ident(), datetime.now(UTC),
        [{'type': 'image', 'data': {'file': filename}}], (filename,)), service)
    ...
    kwargs = dict(db=path, media_dir=tmp_path,
        whitelist=[(dhash64(data), 'db:enabled')], max_distance=2, limit=4000)
    if tool == 'replay':
        ...
        if not accepted:
            ...
            evidence = next((x for x in result['exceptions'] if x['message_id'] == row.message_id), None)
            assert changes or (evidence and 'unresolved_unknown' in evidence['blocked_by']), (low, high, detail, result)
    else:
...
def test_missing_review_evidence_is_not_reported_as_known_clear(shape):
    if shape == 'partial_primary':
        detail = {'ai_results': [{'source': 'vision', 'category': 'ad',
            'confidence': .7, 'model_id': 'synthetic', 'needs_review': False}]}
        ...
        detail = {'ai_results': [{'source': 'vision', 'category': 'ad',
            'confidence': 'not-a-number', 'model_id': 'synthetic',
            'review_role': 'primary', 'review_group': 'a', 'needs_review': False}]}
