"""T-307 NapCat/OneBot 撤回/禁言/警告动作 Adapter 测试。

覆盖规格要求：成功、明确失败、超时、连接中断、权限不足（retcode）、
保护角色、重复事件、重启、急停、按群开关、非数字ID、UNKNOWN不重放。
全部使用假 caller / 假客户端，真实外部调用数为 0。
"""
...
def test_recall_async_accepted_as_success() -> None:
...
def test_explicit_failure_returned_as_failed() -> None:
    caller = _FakeCaller(response={"status": "failed", "retcode": 100, "wording": "权限不足"})
    result = asyncio.run(OneBotActionClient(caller).recall("100", "999"))
    assert not result.ok and result.err_code == 100 and "权限不足" in result.err_message


def test_pre_send_not_ready_is_failed_not_unknown() -> None:
    caller = _FakeCaller(error=OneBotActionError("not_ready", "degraded"))
    result = asyncio.run(OneBotActionClient(caller).recall("100", "999"))
    assert not result.ok and "未发出" in result.err_message
...
def test_non_numeric_message_id_is_failed() -> None:
    ...
    assert not result.ok and "非数字" in result.err_message
...
def test_warn_sends_send_group_msg_with_reply_and_text() -> None:
    caller = _FakeCaller(response={"status": "ok", "retcode": 0})
    asyncio.run(OneBotActionClient(caller).warn("100", "999", "请停止违规行为"))
    ...
    assert msg[1] == {"type": "text", "data": {"text": "请停止违规行为"}}
...
def test_invalid_response_missing_status_propagates() -> None:
    caller = _FakeCaller(response={"retcode": 0})  # 无 status
    with pytest.raises(OneBotActionError) as exc:
        asyncio.run(OneBotActionClient(caller).recall("100", "999"))
...
def test_hub_call_roundtrip_resolves_by_echo() -> None:
    ...
    async def driver() -> dict:
        return await hub.call("delete_msg", {"message_id": 1})
    ...
    assert ws.sent and json.loads(ws.sent[0])["action"] == "delete_msg"
...
def test_hub_unbind_fails_pending_as_response_lost() -> None:
    ...
    async def main() -> None:
        ...
        await asyncio.sleep(0.02)  # 让 send 完成，进入等待
        hub.unbind(ws)
        with pytest.raises(OneBotActionError) as exc:
            await task
        assert exc.value.kind == "response_lost"
...
def test_hub_handle_response_ignores_events_and_unknown_echo() -> None:
    ...
    assert hub.handle_response({"status": "ok", "retcode": 0}) is False  # 无 echo
...
# ---------- 编排器集成（DB + 假客户端，真实外部调用数 0） ----------


class _FakeOneBotClient:
    """实现 ModerationActionClient 协议；记录调用并可按动作失败。"""
...
async def test_official_onebot_success_executes_recall_mute_warn() -> None:
    ...
    assert len(again) == 3
    assert client.calls.__len__() == 3  # 无新增调用
...
async def test_timeout_freezes_unknown_and_no_replay() -> None:
    ...
    assert "禁止自动重放" in intents[0].reason
    assert client.calls.__len__() == 1  # 仅一次 recall 尝试
    # 重复事件不得重放
    async with SessionLocal() as session:
        again = await orchestrate_actions(
            session,
...
async def test_no_route_fail_closed() -> None:
    ...
    msg = _ob_msg(group, "1001")
    async with SessionLocal() as session:
        # 仅启用群动作，不配置路由 → resolve_action_provider 返回 None
        gs = await session.get(GroupSettings, group)
        if gs is None:
...
async def test_action_logs_written_for_audit() -> None:
    ...
    assert all(log.external_group_id == group for log in logs)
