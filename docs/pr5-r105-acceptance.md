...
- 首次本轮CI `34447804238`：Ubuntu和干净运行时通过，Windows在类型检查发现非Windows锁分支未被静态裁剪，未运行该job测试，因而未合并。以 `mypy --platform win32 app` 精确复现3个fcntl属性错误后，平台判断改为sys.platform，移除多余Any绕行；Windows目标与本机71文件类型检查、完整lint/format及55项锁/入站/纠错回归通过。该小修复后的全套远程结果仍以最终head的新CI为准，不隐藏失败运行。
