...
## 八、09-17 负责人口径调整：校园墙图豁免放宽为「口径 C」（主审复验通过，已关闭）
...
主审原始探针修复前 2 failed / 2 passed → **4/4**；相关 6 文件 **66/66**；**已于 09-18
12:01 提权重启生效**（QQBotWeb PID 9604 / QQBotRuntime PID 43536）。

> 主审复验确认（2026-09-18，main@fb7817d）：P1 已修复、**可关闭**；本轮未发现新增阻断
> 问题。原始 `test_same_second_pending.py` **4 passed**（未修改断言）；相关 6 文件
> **66/66**（含新增数据库 processing 同秒回归）；附带后台折叠改动相关测试 **46 passed**；
> 同 SHA CI run 35297671396 三 job（Ubuntu / Windows / 干净依赖）实际执行并通过。
> 修复边界正确：**未完成图片含同秒保护，转人工且无处罚建议；已完成图片同秒仍不授予
> 豁免**；未扩大口径 C 的授权范围。**P1 关闭。**
