...
├─ ONEBOT_ACTION_STAGE=recall_only ← 起步只撤回；批准full并重启才禁言/警告
...
- 触发一条高置信违规（测试群发广告词），观察：
  ...
  - 仅首次违规群内出现警告回复；第二次无警告并生成案件
- 管理后台 → 案件/动作日志可审计每次执行结果
- `action_intents` 表出现 `SUCCEEDED` 状态记录

默认recall_only时只会撤回，不会禁言或警告。W3先验证撤回成功，再经负责人批准改 `ONEBOT_ACTION_STAGE=full` 并重启，验证完整阶梯。此阶段是全局配置，先只启用一个隔离群；W5目标群试运行重新从recall_only起步。仅撤回阶段仍累计有效违规，切full不会清零30天历史，所以应检查成员历史并用新同意的测试成员测首违，禁止删除真实历史凑测试。
...
重启服务。撤回不能恢复原消息，禁言可由授权管理员人工解除；新消息不执行真实处罚。
