...
## STAT-RECALL-20260926：统计大盘显示撤回证据（源码候选）

负责人希望直接在“统计”页看到撤回情况。原页面只汇总消息、违规、案件与 AI；日报/周报虽已有 OneBot 撤回通知计数，却不易在统计页发现。本轮复用报告的 `ActionIntent` 与 `RecallConfirmation` 口径，在统计页单独显示最近 7 天的“进入撤回流程、接口返回成功、收到匹配 QQ 撤回通知、尚无匹配通知、未采集确认”。窗口按意图创建时间、UTC 半开区间计算，并受当前判断记录保留期上限约束；迟到通知会更新仍在窗口内的数字。

“接口成功”和“收到通知”可能重叠，不能相加或合成真实撤回成功率；UNKNOWN/FAILED 也可能后来收到通知。尚无通知、未采集确认均不等于群内消息未撤回。现有系统未对有效但未匹配的通知留无身份原因计数，统计页只能展示已有证据，不能判断 0 条确认的根因，也不声称证明所有 QQ 客户端视觉消失。本轮不新增数据表、外发、群动作或生产迁移；后台保持登录保护，仅显示匿名汇总。

聚合与页面用独立内存库和既有后台路由回归先红后绿验证，旧日报/周报计数回归保持。本机全量 pytest、Ruff/格式、mypy 与差异检查均退出码 0。生产 Web 需独立获授权装载，不能把源码检查当现场生效；实际浏览器显示与 NapCat 通知链仍须上线后核验。
