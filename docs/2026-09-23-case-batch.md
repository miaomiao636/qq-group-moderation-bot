...
- 批量保留结案候选上限已于 CASE-BATCH-5000-20260926 提高到 5000 案（旧版为 200 案，生产加载状态见文末），要求未归档且待审，先列出具体案件供人工核对。CASE-SELECT 补正后 UI 不要求填写原因，默认审计记录为“人工处理”。已关闭重复项及历史证据身份错连案件在预览时分别列明并跳过，后者保持待处理；其他不可处理案件整批拒绝。已有保留期清理不会因结案预览获得新删除授权。
...
初版源码 `28607a8` 的专项 79 项通过。只读生产盘点发现 1583 件待审案中有 21 件涉及 80 条身份不符的历史证据（单次快照，非固定总数），负责人明确选择“跳过异常案件，其余继续”。候选现于预览先分出异常案，列出全部案件号与异常条数，写入仅含案件 ID/数量/固定原因的跳过审计；健康案仍经严格校验建计划，确认名单不含异常案。全部异常时不生成空计划，页面明确“本次未结案”；确认后新增错连仍整批拒绝。无效证据索引、缺失权威身份、异常审批状态与单块证据超限保持原门禁，不捕获所有错误后静默跳过。
