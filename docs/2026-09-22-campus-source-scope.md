...
## 2026-09-23 自然窗口只读核验补充

执行 SHA `6ecb6d28e38b64b597c95edfb1c312f4cd33a049`，策略依赖逐文件匹配已加载源码 `1c17ba09e5bef268a6162864830396e8d36c6107`。仓库根命令 `.venv/Scripts/python.exe -B D:/QQBotAudits/n03-campus-20260922/campus/natural-window-audit.py --out D:/QQBotAudits/n03-campus-20260922/campus/natural-window-20260922-final.json`；脚本摘要、固定基线、观察时间、私有审计自检及完整结果见 [脱敏收据](evidence/campus-template-20260922/natural-window-6ecb6d2.json)。脚本与自检未作为仓库测试入库，不计为新增主审探针或 CI 覆盖。

基线后读取 573 条记录，现存证据复验合格来源 35 条（单独图片 33、普通文字配图 2）；同 OneBot 账号/provider/群/成员且 `0 < delta <= 120` 的配对 10 个。已取得两条真实窗口结果：rowid `30841 → 30842`（图片后接文字，18 秒）、`30851 → 30852`（图片后接图片，1 秒），均持久化精确豁免理由、`record_only`、空处罚建议；按已核实中立身份关系查询均无动作意图。其余为不属于窗口命中的普通记录或语音排除，未将仅时间接近当成生效。

这证明程序在线保存过相应两分钟豁免结果，不证明其他系统/人工没有撤回，也不唯一确定多个合格来源中实际使用哪条。当前来源行不能重构当时来源已完成/可见时间或原始窗口前决定。附文字发图的两个合格来源尚无后续配对，其自然窗口验收仍待样本；边界值等仍按既有离线回归，不冒记为真实边界验收。没有发送测试消息、网络/模型请求、处罚或设置变更。下文较早“完全没有自然配对”仅是当时状态。
