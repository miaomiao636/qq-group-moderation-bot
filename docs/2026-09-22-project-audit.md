...
本轮修复已推送并经负责人授权上线，生产加载源码 `1c17ba09e5bef268a6162864830396e8d36c6107` / `t204-v18`，急停保持开启。独立 QQ 空间巡检不计入本轮主项目验收。
...
负责人随后明确选择“现在上线，保持急停”。本次独立授权覆盖上述源码、备份及 Web/Runtime 短暂重启；不是复用此前校园墙授权。执行 SHA、UTC 时间、命令、私有脚本/收据哈希和脱敏计数见 [部署收据](evidence/project-audit-20260922/deployment.json)。

- 生产从 `06324fedab4baf8d0ee14e09991c5385627a01ed` 快进到上述完整源码 SHA。UTC `2026-09-22T10:22:03Z` 开始正常停服，`10:22:11Z` 两服务已 Running；之后后台登录与健康端点均 200，OneBot ready/online。服务状态的停启时间不等于消息完全无损或完整恢复时间。
- 停服后在决定锁内调用 `backup_sqlite`，私有一致性副本通过 `integrity_check` / `foreign_key_check`，revision 仍为 `e1c7d4b8a902`；没有迁移或恢复覆盖。此本机部署备份不关闭日常异盘/异机备份待办。
- `.env`、提示词文件和配置/群授权/路由/名单/系统设置摘要前后相同；提示词 `t204-v18`、急停 true，通知维持关闭。实际运行文件与 `git show <源码SHA>:<文件>` 逐项一致。
- UTC `10:24:12Z` 的验证命令 `python -B verify-configured-endpoint.py verify` 观察到备份之后自然新增 inbox 4 条均 DONE、shadow 4 条、action 4 条均 SKIPPED。新视觉结果含 v18 来源字段；没有新增合格 mixed 校园墙来源样本，不能声称自然两分钟窗口完整验收。没有发送测试消息或重放历史动作。
- 初次尝试在停服前因 Windows PowerShell 默认编码误读中文路径而终止，原 PID 继续运行；新尝试显式 UTF-8 读取后成功。核验辅助脚本另有默认端口 8000 假设，保留失败日志后改用实际配置端口 8001，未修改生产端口。原失败证据、脚本和成功收据均保留。

源码验证与部署均完成；后续文档提交不改变运行代码。交接时仍按上文命令核验最终文档 HEAD 的独立 CI，不把源码 run 冒充最终 HEAD run。
