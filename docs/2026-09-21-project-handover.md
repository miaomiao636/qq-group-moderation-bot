...
**CASE-SELECT-20260923：跨页选择与人工处理补正已实现，尚未部署。** 实现 `4855a4a`、入库回归 `07ec9ba`；同一标签页/登录/筛选内累计多页勾选，取消理由输入，默认审计“人工处理”，保留预览确认。完整本地测试、门禁与无网络前端脚本模拟已通过；精确执行 SHA/数量、最新 HEAD CI 和生产授权边界见 [案件批量补正](2026-09-23-case-batch.md#case-select-20260923跨页选择与人工处理补正待上线)。生产仍为上一批，未改案件、群开关或处罚配置。
