...
**最新跟进 DAILY-BACKUP-20260922（备份与恢复验证完成，定时任务待管理员注册）**：不加密、排除密码和 Token；源码 7b040b8、真实明文备份与隔离恢复已验证。Windows 管理员进程未成功启动，QQBotDailyBackup 未创建，待负责人重试 UAC 后实跑验收；见 [每日备份](2026-09-22-daily-backup.md)。实时急停 false，主服务未重启；下文 true 属历史部署快照。新备份不需要密钥，换机重填服务凭据；整机演练待窗口，N03 未批准删除。
...
| HEAD | 已验证备份源码 `7b040b8` 已同步本机；最终文档 HEAD 以 git log -1 为准。主服务仍加载审核源码 `1c17ba0`，本轮无重启 |
| CI | 备份源码 `7b040b8` 的 run `35736054871` 三 job 成功，实际 checkout/命令见每日备份记录；最终文档 HEAD 仍须独立核验 |
| 测试基线 | `7b040b8` 执行 uv run --locked pytest --junitxml=<私有证据>/plain-full.xml：2518 passed / 6 skipped / 0 failed/error |
| 主审探针 | 同次全量 JUnit 子集：31 文件 / 262 passed，原探针未改动 |
| 静态门禁 | 同源码 uv run --locked ruff check app tests alembic、ruff format --check app tests alembic（353 文件）、mypy app（122 文件）通过 |
| 生产快照 | 审核源码 `1c17ba0` / `t204-v18`，最新只读急停 false；本轮原进程和群授权/动作未变，备份任务尚未注册 |
...
**最新 DAILY-BACKUP-20260922**：源码 7b040b8 的全量、门禁、精确 CI 与真实明文完整隔离恢复已通过；当前定时任务尚未注册，管理员进程启动失败后等待负责人重试 UAC。完整 SHA、命令、收据和原进程/开关不变比较见 [每日备份](2026-09-22-daily-backup.md)。后续仅文档提交不代表自动备份已启用。
...
**DAILY-BACKUP 当前待办**：代码与真实明文备份/完整隔离恢复已验证；QQBotDailyBackup 尚未注册（Windows 管理员进程未成功启动）。负责人选择重试 UAC 后，仅执行已准备的注册助手，触发任务并核对 SYSTEM 成功收据，再更新本条。保持现有急停/动作、原主服务进程和旧证据；新备份不需要恢复密钥。
