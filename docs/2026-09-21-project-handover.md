...
**CAMPUS-TEMPLATE-20260922 候选修正**：负责人批准恢复认可分享卡模板；`t204-v18` 联合来源观察及图片/窗口共享门禁已实施，验证与生产状态见 [本轮记录](2026-09-22-campus-source-scope.md)。线上仍为 `t204-v17`，急停未改。
...
**CORE-CLOSEOUT 最终源码 `59d15eceff32df54520d214b04bd0dc9647c7f63`**：全量/门禁/主审子集与 N03 新观察的命令及数字见 [收尾记录](2026-09-22-core-closeout.md)，下方较早数字为历史快照。A2 是独立候选，未在生产部署。
...
| # | 事项 | 状态 / 前置 | Owner |
| --- | --- | --- | --- |
| 1 | **方案 A 提案**（同库权威保存 + JSON 可重试导出）：`docs/2026-09-20-r132-image-decision-authority-proposal.md` | 负责人已授权并完成裁定、独立候选实现及隔离验证；生产迁移仍需窗口 | 本任务审查＋实施 |
...
| 8 | 异常账号巡检 | 负责人要求**继续研究自动识别，不实施名单匹配**。手机辅助抽样已读到异常提示及对应 QQ；全群自动遍历、覆盖与恢复尚未实现。执行证据和边界见 §7.1 | 我方；先验证独立手机辅助工具 |
