...
**CAMPUS-TEMPLATE-20260922 已实现、未生产加载**：源码 `ff4d1a2` 已推送；认可分享卡联合识别、图片与两分钟窗口共享门禁已完成本机及授权原图复验。执行 SHA、命令、CI 和原始样本边界见 [本轮记录](2026-09-22-campus-source-scope.md)。线上仍为 `t204-v17`，急停未改。

**CAMPUS-SCOPE-20260922 已上线**：仅明确确认的万能校园墙来源保留豁免，通用小程序码恢复常规审核。加载 `8f4617edf6f73d8b98683994d575b022b9f2a0e4`、提示词 `t204-v17`；健康、新视觉记录、测试和 CI 的 SHA/命令见 [记录](2026-09-22-campus-source-scope.md)。

**A2-DEPLOY-20260922 已上线**：负责人明确授权“方案 A 上线”；真实备份副本验证后，已正常停服、迁移/回填、身份对账并恢复服务。当前加载源码 `53683d5998588f3b7490f4e73774df313b17b51d`，新 revision `e1c7d4b8a902`。命令、证据和未完成边界见 [上线记录](2026-09-22-authority-a2-deployment.md)。此前“生产未部署/待窗口”为历史状态；整机恢复与 N03 仍待办。

> **A2 独立候选分支**：已裁定并实现同库决定和可重试导出。执行 SHA、命令、主审适配与隔离恢复见 [候选验收](2026-09-22-authority-a2-validation.md)。本分支有新 migration，生产工作分支继续旧 head；没有生产迁移、部署或开关变更。不要将本分支的代码存在等同于正式启用。
...
| CI | 分页最终 `4571e0a` 的 run `35574880076` 已成功；后续稳定性首推 `bfc027f` 对应 `35577877038`，证据见 [整改记录](2026-09-21-longterm-stability.md)；最终 HEAD 须单独核对，不能借用旧结果 |
| 测试基线 | 后续执行 `6806210`：**1955 项 / 1939 passed / 0 failed / 0 error / 16 skipped**；命令与原因见 [整改记录](2026-09-21-longterm-stability.md) |
| 主审探针 | 同次全量 JUnit 子集 `tests.test_r132_review_*`：**31 文件 / 262 项**全部通过；本轮未修改 |
| 静态门禁 | 后续执行 `6806210`：`ruff check` ✓ / `ruff format --check` ✓（**314 文件**）/ `mypy app` ✓（91 文件）；完整命令见整改记录 |
...
**2026-09-22 模板修正最新状态**：冻结源码 `ff4d1a2aa79f3ff2f6e88a1ef5acafe262fc731e` 已推送；本机全量、原主审探针、门禁、负责人授权原图模型复验及原图离线管线回放均有独立证据，完整执行命令与数字见 [模板验收](2026-09-22-campus-source-scope.md#本轮验证与交付边界)。本轮没有改变生产代码、配置或服务，线上仍为 `t204-v17`，急停保持 true。隔离候选 worktree 为 `%LOCALAPPDATA%/Temp/qqbot-campus-template-20260922`；不要把远程分支的新代码等同于生产进程已经加载。
...
- **校园分享卡模板修正待加载**：`t204-v18` 已实现并完成实样验证；新图将按联合模板来源判断，后续窗口沿用同一资格。生产仍 `t204-v17`。后续按本轮验收记录的正常发布流程加载，保持急停；解除急停属于负责人操作，不由本轮测试结果自动触发。不重判或补罚历史消息。
...
**CAMPUS-TEMPLATE-20260922**：本轮仅新增 `tests/test_campus_share_template.py`，没有适配、删除或更改既有主审探针；真实原图与响应仅在私有目录留存，公开验证收据见本轮记录，不计作入库测试。下列为更早历史登记；上一轮 CAMPUS-SCOPE 的适配与封存原件见 [来源范围记录](2026-09-22-campus-source-scope.md#回归与原探针适配)，不是本轮新增适配。
