...
> **后续已选**：负责人已选择：正常访问恢复后验证低频分批。桌面现为每批最多 10 人、批内间隔 30 秒、批末自动停止且手动续批；这是实验参数，不是腾讯安全频率。尚未进行恢复后的真实批次验证。 GitHub 研究结论见巡检使用记录，不把第三方接口库当成已验证的异常检测器。

> **当前限制**：整群连续扫描已遭腾讯 WAF 拦截，整群一次扫完验收未通过。已有进度和结果保留，程序明确停止而不把拦截算作成员异常；平台恢复及低频分批可行性尚未验证，不能交接成稳定全群批量工具。详见使用记录 WAF 段的执行 SHA、命令和实机证据。
