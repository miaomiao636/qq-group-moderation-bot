...
**最新跟进 DAILY-BACKUP-20260922（实施中）**：完整运行数据备份已获授权；实现、调度与验收进度见 [每日备份](2026-09-22-daily-backup.md)。最新只读急停为 false，保持当前实值；下文 true 属历史部署快照。主服务不重启；Windows 整机演练仍待窗口，N03 未授权删除，恢复密钥尚需负责人异机保管。
