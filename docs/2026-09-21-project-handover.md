...
> **CORE-CLOSEOUT-20260922（主项目收尾）**：报告一致快照、UTC/区间与证据防覆盖修复已入库；已补固定窗口 shadow 报告、当前参数 B/C Mock 复测、合成库恢复和 N03 只读预检。具体 SHA、命令、数字与边界见 [收尾记录](2026-09-22-core-closeout.md)。主服务保持运行，通知按负责人最新决定暂不开启；N03 原件处置须先审核清单。负责人已授权本任务承担方案 A 审查和实施，不再等待外部裁定；方案 A 将隔离实现，未授权生产迁移/整机停机。
...
| # | 事项 | 状态 / 前置 | Owner |
| --- | --- | --- | --- |
| 1 | **方案 A 提案**（同库权威保存 + JSON 可重试导出）：`docs/2026-09-20-r132-image-decision-authority-proposal.md` | 负责人已授权本任务审查和实施；先修正回滚/历史保存方案，隔离实现，生产迁移仍需窗口 | 本任务审查＋实施 |
...
外部输入仍包括：**方案 A 的裁定**、**回滚演练的窗口**；分页与稳定性修复已随 MEDIA-QUOTA 授权恢复加载，证据见恢复记录。当前新增的电脑空间限制巡检及其未验证边界见 [本轮记录](2026-09-21-space-inspector.md)，不以旧手机路线作为前提。
