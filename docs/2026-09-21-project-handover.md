...
| HEAD | `2e9e686`（docs: 方案 A 独立提案）；工作区**干净** |
...
| 静态门禁 | `ruff check` ✓ / `ruff format --check` ✓（**307 文件**）/ `mypy app` ✓（89 文件） |
...
- **166 个测试文件、1895 用例**（Windows 本机：15 skipped 属平台差异；主审 macOS 为 1 skipped）。
...
3. **交接检查单**（每次交接前逐项确认）：
   ...
   - [ ] 全量 1895 用例 0 failed / 0 error；262 项主审探针通过
...
代码与测试基线是稳的（262 项外部契约全绿 + 全量 1895 全绿 + 三次 CI 全绿）；
**真正的工作面是"与主审的证据对话"**：复现 → 原样入库 → 登记式适配 → 小步整改 → 可复算的口径。
当前就两件事等外部输入：**方案 A 的裁定**、**回滚演练的窗口**。
