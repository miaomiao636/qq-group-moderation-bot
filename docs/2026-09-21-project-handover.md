...
**最新 CASE-BATCH-20260923：源码已推送，未部署。** 冻结源码 `d3e2d3c` 新增案件摘要 CSV 导出和预览后批量保留结案；不处罚成员、不撤销违规、不恢复硬删除。补正 NapCat 撤回回执超时为 UNKNOWN，禁止自动重放；历史 FAILED 不回写。全量、原主审、门禁与合成界面验证及 CI 核验入口见 [本轮记录](2026-09-23-case-batch.md)。本轮未重启生产、未改群开关、未处理线上案件；最近验证的生产源码仍为下方 `23d0a1f`。N03 工程闭环、具体处置授权和整机演练仍独立待办。
...
**CASE-BATCH-20260923**：源码 `d3e2d3c` 的全量、原主审、门禁与合成界面已核验；本轮仍未部署。精确执行 SHA、命令、数量、初版失败整改和 CI 核验入口见 [案件批量与回执超时](2026-09-23-case-batch.md)。下方已上线卡片版本仍是生产最近验证状态。
...
**CASE-BATCH-20260923 部署待办**：本批源码与验证已完成，生产重启需负责人针对本批明确授权；先在 D 盘备份，保持当前急停、recall_only 和群开关。不得代用户批量结案。N03 来源期限权威索引、全副本继承、替代备份及具体处置清单批准未完成，不能因本轮功能通过而关闭。
