...
**最新跟进 DAILY-BACKUP-20260922（实施中）**：完整运行数据备份已获授权；实现、调度与验收进度见 [每日备份](2026-09-22-daily-backup.md)。最新只读急停为 false，保持当前实值；下文 true 属历史部署快照。主服务不重启；Windows 整机演练仍待窗口，N03 未授权删除，新备份不使用恢复密钥，换机重填服务凭据。
...
| HEAD | 已验证源码 `1c17ba0`；最终文档 HEAD 以 `git log -1` 为准。生产已授权加载 `1c17ba0`；后续仅文档同步不代表再次重启或改变加载源码 |
...
| 生产快照 | `1c17ba0` / `t204-v18` 已加载、急停 true；本轮 PROJECT-AUDIT 部署和自然消息核验已完成 |
