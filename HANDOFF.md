...
## 当前交接：2026-09-15，R-111 / v13.1 两轮 P2 复验修复（**主审复验通过**；最新 `d388ab5`）
...
**真实部署状态（2026-09-15 已部署）**：负责人授权后已部署 `d388ab5`（工作树 `8ae6fdb`，仅文档差异）——服务重启后 healthz `ok / ready / connected`；规则 `t204-v13` + rules sha256 `8608ABC2…EE5D6`；迁移 `af66bc20f1ed (head)`；`ONEBOT_ACTIONS_ENABLED=false`、`action_intents` 全 `SKIPPED`（5 个历史授权群仅存配置）。**生产后台实测通过**（真实记录两次纠正 + 只改原因不回退 + 审计 +3 + 零副作用；同证运行版本已含修复）。**恢复真实动作、生产删除、Git 历史重写均未授权、未执行**；未合并 main。详见 `docs/evidence/2026-09-15-deploy-d388ab5.md`。

**待办**（2026-09-15 更新）：
- **PR #7 已于 2026-09-15 合并 main**（合并提交 `77ecaf3`，负责人授权）。
- ~~真实 TUN 下载实测~~：已取得 Fake-IP 环境真实证据（QQ CDN 解析 198.18.0.71；最近图片消息媒体全部下载成功），见部署证据文档，待主审核验关闭。
- **独立新 W2 → 改为"线上抽样复测"**（负责人 2026-09-15 采纳替代方案）：在线运行自然积累后，从线上抽 100–200 条未参与调参样本评测；"98%"仅作开发集数字，不作通例承诺。
- **百群负载与延迟**：**基线 + 压测 + 优化建议全部完成**（`docs/evidence/2026-09-15-capacity-baseline.md`、`docs/evidence/2026-09-15-capacity-loadtest.md`）：单实例处理上限 ≈**90–100 条/分钟**；百群日常（≈14 条/分钟）富余 6–7 倍；极端峰值（120–140 条/分钟）超出→限流降级转人工（不处罚不漏放）+积压 20–50 条自愈。**必改项：`AI_DAILY_CALL_LIMIT` 3000→25000+（百群日需 ≈2.3 万）**；建议：`AI_PER_MINUTE_LIMIT` 90→150–200、worker 并发 3→6。压测工具 `scripts/capacity_loadtest.py`（隔离库+mock AI，可复现）。
- 隔离副本清理/恢复验收、通知接管/整机故障演练、授权后的 W3–W5。
