...
**RECALL-ONLY-20260925（源码与本地验证完成，生产待加载）**：负责人要求彻底取消首次/再次违规自动禁言与群内警告；本轮同步停用第二次违规自动立案，新自动动作仅撤回。OneBot/官方出站适配器已删除禁言与群内警告方法，完整处罚配置失效；旧动作意图不可续罚。违规证据、既有案件与历史动作审计保留。全量 pytest 退出码 0，Ruff 检查/格式、mypy 与差异检查通过；提交/推送、远端 CI 和生产加载须另记结果。本轮未修改生产配置或历史证据；旧记录中对 `full` 档位和禁言/警告试运行的描述只作历史，现行口径以 D-041 为准。
