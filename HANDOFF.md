...
**CASE-REOFFEND-BATCH-20260925（已上线，真人操作待验收）**：D-042 覆盖 D-041 的自动立案禁令，不改变仅自动撤回的群动作。跨页批量人工处理可跳过已关闭的重复选择；确认结案后同群成员再犯另开人工待审案，旧案/证据保留；误判结案不视作已确认违规。已关闭案仍在默认列表，满 15 天由维护任务归档，满 90 天且关联安全时仅精简非必要内容，保留编号、审计和前案引用。完整 pytest、Ruff/mypy、精确源码 CI 三 job、负责人授权及 D 盘备份后双服务重启、OneBot 恢复已核验；实机跨页与自然再犯仍待真人验证。执行细节见 [本轮记录](docs/2026-09-25-case-reoffend-batch.md)。旧 RECALL-ONLY 条目为此前阶段口径。

**RECALL-ONLY-20260925（历史阶段）**：负责人要求彻底取消首次/再次违规自动禁言与群内警告，当时同步停用第二次违规自动立案。OneBot/官方出站适配器已删除禁言与群内警告方法，完整处罚配置失效；旧动作意图不可续罚。违规证据、既有案件与历史动作审计保留。当时全量 pytest、Ruff、mypy 与差异检查通过；生产加载仍须另记结果。自动立案口径现以 D-042 为准，旧 `full` 档位和禁言/警告试运行描述只作历史。
...
**最新 CARD-RECALL-20260923 已推送并上线**：源码 `23d0a1f` 补正实际群名片结构，并将微信小程序分享卡纳入撤回；保留保护角色、成员白名单与校园墙二维码图片政策。五个缺失群已授权启用，绵阳免费家教群 `628717263` 按负责人最新要求审核/动作双关闭。生产已正常重启、OneBot 恢复与自然业务核验通过；急停 false、动作阶段 recall_only。执行 SHA、全量/门禁/CI、群数量、D 盘备份及具体实机边界统一见 [本轮记录](docs/2026-09-23-card-recall.md)。本条覆盖下文旧版“当前加载 1c17ba0 / 急停 true”；最终文档 HEAD CI 仍按 §11 精确核查。

## SPACE-EXPORT-20260923：导出布局与群结果辨识

本轮仅修改 `app/space_inspector/` 与内部回归。根代理为唯一写入者，辅助审查只读；不改主项目生产状态。新导出在桌面“QQ空间巡检结果”，内部任务编号及旧文件保留。重开窗口、载入原任务并再次导出即可使用，不需要重扫；负责人选择稍后验证，本机真实任务再导出反馈尚未取得，不以隔离夹具冒充。代码/回归 SHA、验证命令和 CI 核验入口见 [巡检记录](docs/2026-09-21-space-inspector.md#导出目录与群结果辨识space-export-20260923)。接手先 pull/status，仍按唯一入口 §11 核对精确 HEAD CI。

**N03-CAMPUS-20260923**：本轮完成只读清单及线上窗口补查，原件与生产行为不变。私有审核清单在 `D:/QQBotAudits/n03-campus-20260922/README.md`；仓库提交脱敏收据和记录，不把私有审计脚本当入库测试。执行 SHA `6ecb6d2`、完整命令、初稿修正和存储范围见 [N03 补查](docs/2026-09-22-core-closeout.md#2026-09-23-处置清单补查只读未批准删除)，真实图片后接文字/图片窗口结果与 mixed 缺口见 [线上窗口](docs/2026-09-22-campus-source-scope.md#2026-09-23-自然窗口只读核验补充)。下一步是原消息固定期限与未知历史口径、合规替代快照和具体删除审批；整机演练仍待窗口。没有完整恢复副本写入 C 盘。

**DAILY-BACKUP-20260922 清理补充**：负责人授权清理的三份 C 盘完整恢复副本已删除，清单/收据另存且 E 盘备份保留。以后完整恢复验收使用 D/E 盘；本次仅清理副本并更新规则、恢复示例和交接，未改代码或重启服务。执行 SHA、核验命令、释放空间与保留边界见 [清理记录](docs/2026-09-22-daily-backup.md#c-盘恢复副本清理与后续位置2026-09-22)。旧 C 盘恢复路径不可再直接使用，N03 和整机演练仍待授权/窗口。
...
## SPACE-INSPECT 最新追加范围

非好友维护页补丁已获负责人重开续扫反馈，并只读确认待确认结果落盘、原暂停历史保留且继续下一成员；完整群覆盖及新增 GUI 导出未据此验收。详见巡检记录本补丁实机段。
...
最新阻塞：整群连扫遭腾讯 WAF 拦截，不能交接为稳定一次性全群工具。代码已明确识别并停止，已有进度保留；不继续连扫，不承诺恢复时长或低频一定有效。完整证据与执行 SHA/命令见下方使用记录的 WAF 段。

负责人已接受电脑 QQ 空间限制巡检首版，日常不接手机，未命中待确认。代码与测试已入库并创建独立桌面入口。使用、SHA/命令、私有实页证据口径及验收缺口见 [记录](docs/2026-09-21-space-inspector.md)。后文手机方案是历史阶段；不要继续把连接手机作为本版交付条件。方案 A 和回滚演练待办不变。

当前跟进点：权限页及“对方未开通空间”修复后的有界实机续扫均已取到保存记录；已有 GUI 导出已核对，新补丁后的新增结果导出尚待核验。任务目录可直接打开，暂停会显示具体原因。不要把部分实机、组件导出或隔离验证写成全群验收；数量与执行 SHA/命令见本轮记录。

## 最新入口（2026-09-21）

**最新恢复**：MEDIA-QUOTA-20260921 已获负责人同意；媒体容量改为可配置并在本机设为 20 GiB，在线备份后已正常重启 Web/Runtime，OneBot 重新就绪。分页和稳定性代码也随本次加载。验证及限制见 [恢复记录](docs/2026-09-21-media-quota-recovery.md)；下文“尚未重启/等待授权”是此前历史状态，不再作为当前结论。手机巡检仍有实机验收待办，不因主仓库测试通过宣称完成。
...
异常账号研究的后续制作 PHONE-INSPECT-20260921：负责人拒绝名单匹配，已授权独立手机辅助工具。`app/phone_inspector/` 和对应内部回归已入库，桌面快捷方式已创建；实现按当前群逐成员读取、受控翻页、证据保存、暂停/恢复及导出。实际测试、执行 SHA、命令、原始证据入口及未验证边界见 [使用与验证记录](docs/2026-09-21-phone-inspector.md)。恢复从顶部重核，不是直接跳到断点；完整群覆盖仍须单独验收。下文保留历史记录，不作为本次部署事实。
