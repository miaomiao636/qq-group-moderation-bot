...
**CASE-BATCH-5000-20260926（源码及专项验证完成，尚未生产加载）**：根代理唯一写入，辅助只读复核；修改 `case_batch.py`、`cases/service.py`、`routes.py` 与内部批量回归。5000 案分块读证据、完整核验后统一刷新和提交；负责人另选在预览中明确跳过历史身份错连案件，异常案仍待处理、证据不变。确认不临时跳过新增异常。82 项专项与 Ruff/格式/mypy 通过；最新完整回归及精确 HEAD CI 读取本轮 `checks.json`、`pytest-final.txt`、`ci-final.json`，入口见 [批量记录](docs/2026-09-23-case-batch.md#case-batch-5000-20260926批量人工处理扩大到-5000-案)。无迁移、生产案件处理或服务重启；上线仍须本轮明确授权和 D 盘新备份。全页 CSV 导出及用户核对消息的实际撤回已有真人反馈，后台通知关联仍待修。

**STAT-RECALL-20260926（统计页撤回概览，源码候选，未装载生产）**：负责人要求直接在“统计”页查看撤回统计；新增最近 7 天独立卡片，分别计进入撤回流程、API 成功、匹配 QQ 通知、尚无通知及未采集确认，按意图 UTC 创建时间并受确认保留期约束。接口与通知证据可重叠，不给出误导性“真实撤回成功率”；无成员明细、迁移或群动作。独立内存库聚合与后台页面回归先红后绿，相关撤回日报回归保持；本机全量 pytest、Ruff/格式、mypy 与差异检查均通过。精确 CI 与生产 Web 加载仍须分别核验。现有 0 条匹配通知的根因仍需无身份接收/匹配计数和手机端核验。细节见 [撤回确认记录](docs/2026-09-24-recall-confirmation.md#stat-recall-20260926统计大盘显示撤回证据源码候选)。

**配套交付 CI 补正**：首个 `8fc48d2` 候选在 Windows 历史任务数量测试失败，未宣称远端通过。已复现连接准备吞掉 SQL 预算的有效任务漏列问题，修正起点且保留原查询中断门禁；内部目录排序用例固定时钟，数量/顺序断言不变，外部主审探针不动。最新完整执行、最终包与精确 HEAD CI 以 [交付整改](docs/2026-09-25-companion-delivery.md#companion-ready-20260926) 的 `recheck`、`final-bundle.json` 和 `ci-final.json` 为准，不能沿用失败候选。无生产操作。

**COMPANION-READY-20260926**：负责人“继续”后核对“新状态”空闲及 `0c82cd4` 干净同步，由本任务根代理唯一写入、辅助只读审查。改动集中于安装脚本、主备份发行来源验证、巡检备份、回归与交付文档；无生产操作。公司登录、实际服务故障恢复/备份调度和长期巡检仍待新现场验收。精确源码、命令、包与最终 CI 证据以 [交付整改](docs/2026-09-25-companion-delivery.md#companion-ready-20260926) 为准，禁止沿用前轮数字或把代码回归当实机通过。D-035 已明确 MIT/单公司 ZIP，无须再次索取许可决定。

**CASE-ARCHIVE-30D-20260926（D-043，已获授权上线；次日清理和真人页面待核验）**：负责人指定已关闭案件满 30 天再进入归档。Codex 单独修改维护阈值、后台说明、边界回归与现行记录；不改原文 15 天期限、归档后 90 天有条件精简、已归档案件或群动作。固定时钟回归先红后绿；本机全量 pytest、Ruff、mypy、差异检查及 `02fe0c2` 的[同 SHA 三项 CI](https://github.com/miaomiao636/qq-group-moderation-bot/actions/runs/36213970777) 全部成功。D 盘在线备份通过完整性及外键校验后，经负责人明确授权于 2026-09-26 11:30 北京时间只重启 Web；OneBot 已恢复 ready/online、队列 0 并处理新消息，Runtime、急停、群开关保持原样。未手动清理或迁移。下一次 04:00 维护任务和真人案件页文案仍待核验；执行、备份与回滚依据见 [本轮记录](docs/2026-09-26-case-archive-30d.md)。

2026-09-26 11:07 北京时间只读快照：Web/Runtime 服务运行且 OneBot ready/online；75 个 OneBot 群配置中 71 个审核和动作同开、4 个同关，动作阶段 `recall_only`、数据库急停关闭。生产库有 1,515 件未归档待审、2 件未归档已关闭、0 件已归档；故延长已关闭案件归档期不会减少待审积压。媒体使用 3.615/20 GiB；当日 04:00 清理和 04:30 E 盘备份成功。撤回确认表 1,599 条中匹配的 QQ 撤回通知仍为 0，不能把 API 成功等同客户端确认。此为单次运行快照，后续需重新核验。

**CASE-REOFFEND-BATCH-20260925（已上线，真人操作待验收）**：D-042 覆盖 D-041 的自动立案禁令，不改变仅自动撤回的群动作。跨页批量人工处理可跳过已关闭的重复选择；确认结案后同群成员再犯另开人工待审案，旧案/证据保留；误判结案不视作已确认违规。已关闭案仍在默认列表，当时满 15 天归档的等待期现由 D-043 延至 30 天；满 90 天且关联安全时仅精简非必要内容，保留编号、审计和前案引用。完整 pytest、Ruff/mypy、精确源码 CI 三 job、负责人授权及 D 盘备份后双服务重启、OneBot 恢复已核验；实机跨页与自然再犯仍待真人验证。执行细节见 [本轮记录](docs/2026-09-25-case-reoffend-batch.md)。旧 RECALL-ONLY 条目为此前阶段口径。
