...
R-101已通过；W0已通过；**T-001 可验证项全部完成**（D-012/D-013，仅剩限流格式一项低风险观察项）；**T-002 核心规则已全部澄清**（2026-09-05第四轮）：白名单最终规则=图片带「万能校园墙」小程序码即允许（8张样本负责人逐张判定：带码4张允许、无码4张违规）、刷屏=1分钟>5条且相同字样/图片/表情包、色情/暴力类识别交内容安全服务商（待负责人确认该口径）。规则基线 `docs/group-rules.md` 已完整。**T-102 适配器开发是当前下一步**；T-002 转入持续补样本（不再阻塞）。
...
- **T-002 待补（持续，不阻塞开发）**：①各类别样本距离验收数量（≥20正例+30反例，媒体各≥10）差距大，负责人持续提供；②色情/暴力类「由内容安全服务商评测集承担」的验收口径调整待负责人确认（确认后补决策）。
