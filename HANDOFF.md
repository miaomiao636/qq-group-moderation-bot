...
**CASE-BATCH-5000-20260926（验证中，尚未生产加载）**：根代理唯一写入，辅助只读复核；修改 `app/web/case_batch.py`、`app/cases/service.py` 和 `tests/test_case_batch.py`。5000 案上限与既有跨页选择一致，200 案一块读取证据后完整核验，延迟刷新和批量审计缩短写锁，仍单笔提交及整批失败回滚。79 项批量回归、Ruff/格式/mypy 通过，全量回归和精确 HEAD CI 须继续核验；无迁移、生产案件处理或服务重启。负责人已真人确认全页 CSV 导出成功，以及其核对群消息实际撤回；后台 QQ 通知关联仍待修，不以 0 条关联否认用户观察。下一步与验证收据见 [批量记录](docs/2026-09-23-case-batch.md#case-batch-5000-20260926批量人工处理扩大到-5000-案)。
