...
**CASE-BATCH-20260923：源码已推送，未部署。** 负责人选择批量导出＋批量结案（不处罚）；源码 `d3e2d3c` 复用 KEEP/CLOSED 与现有确认计划表，补充会话/CSRF、冻结反向证据、并发幂等和整批回滚。NapCat 撤回回执超时改为 UNKNOWN、不重放。全量/门禁/原主审与合成界面结果及最终 HEAD CI 核验入口见 [本轮记录](docs/2026-09-23-case-batch.md)；不借用旧 CI。根代理唯一写入，其他代理只读，未改主审探针。上线前按唯一入口 §10 第 2 条取得本批生产重启授权；N03 与整机故障恢复仍未关闭。
