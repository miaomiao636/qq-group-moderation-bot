...
**CASE-REOFFEND-BATCH-20260925（本地验证完成，生产待加载）**：D-042 覆盖 D-041 的自动立案禁令，不改变仅自动撤回的群动作。跨页批量人工处理可跳过已关闭的重复选择；确认结案后同群成员再犯另开人工待审案，旧案/证据保留；误判结案不视作已确认违规。已关闭案仍在默认列表，满 15 天由维护任务归档，满 90 天且关联安全时仅精简非必要内容，保留编号、审计和前案引用。完整 pytest、Ruff 检查/格式、mypy 与差异检查通过；GitHub CI 和生产加载仍需单独核验。旧 RECALL-ONLY 条目为此前阶段口径。
