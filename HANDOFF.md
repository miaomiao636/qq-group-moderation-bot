...
**T-306 NapCat/OneBot入站Adapter与影子运行器：独立主审发现的阻塞问题已在分支 `feature/t306-onebot-shadow` 本地整改并通过全量门禁，等待整改提交的远程跨平台CI，尚未最终验收。** CI成功后进入 T-303（Windows隔离群W1影子验证），随后 `T-307 → T-404 → T-403`；T-304人工批准踢人为T-307之后的可选增强。
...
- 仍待：将整改提交推送到PR #3并取得该提交Ubuntu、Windows和干净运行时三项CI成功证据。CI前不得宣布T-306通过，也不得开始T-303。
