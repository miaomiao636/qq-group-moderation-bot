...
**CASE-BATCH-5000-20260926（已授权上线）**：生产 Web 已加载 `4e2a578`，每次最多 5000 案；预览明确列出并跳过已关闭及历史证据身份错连的案件，异常案仍待处理、原证据不变，确认阶段继续严格校验冻结名单。本机 2806 passed / 19 skipped、82 项专项与源码三项 CI 成功。D 盘新备份校验后仅重启 Web，新进程、OneBot ready/online、自然消息恢复及配置不变均核验；Runtime 原进程保持运行。无迁移，未代用户处理生产案件。执行 SHA、命令及收据见 [批量上线记录](docs/2026-09-23-case-batch.md#批量-5000-授权部署2026-09-26)。用户刷新案件页后可使用；真实全页 CSV 导出与用户核对消息的撤回已有反馈，后台通知关联仍单独待修。
