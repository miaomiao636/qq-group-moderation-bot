...
- **百群负载与延迟**：**基线 + 压测 + 优化 + 参数调整 + 复验全部完成**（`docs/evidence/2026-09-15-capacity-baseline.md`、`...capacity-loadtest.md`）。**已按负责人授权调整**（提交 `dd2494c`）：`AI_DAILY_CALL_LIMIT` 3000→**50000**、`AI_PER_MINUTE_LIMIT` 90→**600**、`_WORKER_CONCURRENCY` 3→**10**；复验（唯一文本最严苛）：120 与 **240 条/分钟均零限流、零积压**（积压峰 3/8、排空 5s、e2e p95 6.1/5.7s）；10 路并发 SQLite 无锁错误。**服务已重启加载**（`dd2494c`，healthz ok/ready/backlog 0）。
