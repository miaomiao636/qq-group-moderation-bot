...
**RECALL-CONFIRM-20260924：撤回确认已实现，本地全量验证通过，准备部署，尚未上线。** 保留 API 回包语义，新增持久化 QQ 撤回通知关联与未确认不升级处罚门禁；迁移、报告和备份兼容一并验证。此前案件跨页勾选补正随本批待部署，生产仍原版本、recall_only，未改群授权。精确 SHA、验证、两处迁移元数据适配、部署/恢复要求见 [本轮记录](docs/2026-09-24-recall-confirmation.md)。
