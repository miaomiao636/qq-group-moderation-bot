...
**CASE-ARCHIVE-30D-20260926（D-043，待远端及现场生效核验）**：负责人指定已关闭案件满 30 天再进入归档。Codex 单独修改 `app/reports/cleanup.py`、`app/web/routes.py`、边界回归与现行说明，辅助代理仅只读审查；不改原文 15 天期限、归档后 90 天有条件精简、已归档案件或群动作。固定时钟回归先复现 15 天误归档，再验证 30 天整点、未满、待审和已归档；相关清理/页面与本机全量 pytest、Ruff、mypy、差异检查均退出码 0。提交/精确 CI、下一次 04:00 清理结果及 Web 新文案需分别核验；本批未执行生产重启、迁移或清理。生产重启遵循 `docs/2026-09-21-project-handover.md` §10 的明确授权与备份要求。
