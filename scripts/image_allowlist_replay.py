...
def load_whitelist(samples_dir: Path, db: Path, media_dir: Path) -> list[tuple[int, str]]:
    ...
    R6-02：本函数只做**生效名单评估**；生效名单不可读/为空时给出的候选**必须由调用方
    在报告里显式标注模式与读取状态**（`effective_state` 的 reason），不得与生效评估混为一谈。
    ...
    # **− 拒绝快照**（R9-06：负责人明确撤回的图不得因"表缺失"重新进入候选集合）。
...
def render(result: dict[str, object], *, window: str, max_distance: int) -> str:
    ...
    lines += [
        ...
        "## 判读方式",
        "",
        "- `会改变结论的` 为 0 只说明**本窗口内样本不再受模型波动影响**；"
        "**enforce（命中即放行）尚未实现、未复验、未获负责人授权**，本文件不构成启用依据；",
