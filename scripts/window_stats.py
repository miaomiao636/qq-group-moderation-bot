...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help=(
            "窗口起点（UTC，含）。默认=**新进程启动时刻** 2026-09-18T17:01:22Z（9ca1066 部署；"
            "主审 A09：旧默认 17:00:00 会含部署前 82–88 秒）。⚠️ 这只是**可证明已加载**的最早时点，"
            "**不等于**应用已就绪：同批服务另有一个 17:01:28 启动，前 6 秒尚无逐条版本归属证明，"
            "该段按未证明处理——若要严格验收，请用**部署后单独窗口**或提供该 6 秒的无记录证明。"
        ),
    )
