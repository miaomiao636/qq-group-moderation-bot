...
def _is_group_card(card: Mapping[str, Any]) -> bool:
    ...
    if isinstance(meta, dict):
        ...
        if isinstance(group, dict) and any(
            ...
            return True
    if app == "com.tencent.qun.share" and view == "group":
        return True
    # CARD-RECALL-20260923: actual QQ Ark generated by the local client.
    # Contact cards also represent people; only a unique group destination qualifies.
    contact = meta.get("contact") if isinstance(meta, dict) else None
    if app != "com.tencent.contact.lua" or view != "contact" or not isinstance(contact, dict):
        return False
    raw_url = contact.get("jumpUrl")
    if (
        not isinstance(raw_url, str)
        or len(raw_url) > 8192
        or any(ord(c) <= 32 for c in raw_url)
        or "\\" in raw_url
    ):
        return False
    try:
    ...
    return (
        url.scheme == "mqqapi"
        and url.netloc == "card"
        and url.path == "/show_pslcard"
        and not url.fragment
        and query.get("card_type") == ["group"]
        and len(uins) == 1
        and re.fullmatch(r"[1-9][0-9]{4,11}", uins[0]) is not None
    )


def _is_wechat_miniprogram(card: Mapping[str, Any]) -> bool:
    """Recognize the observed WeChat Ark schema, not generic miniapp text or images."""
    meta = card.get("meta")
    miniapp = meta.get("miniapp") if isinstance(meta, dict) else None
    return bool(
        card.get("app") == "com.tencent.miniapp.lua"
        and card.get("view") == "miniapp"
        and isinstance(miniapp, dict)
        and miniapp.get("tag") == "微信小程序"
        and miniapp.get("tagIcon") == "https://miniapp.gtimg.cn/public/miniwx.png"
    )
...
def _group_card_name(card: Mapping[str, Any]) -> str:
    ...
    contact = meta.get("contact") if isinstance(meta, dict) else None
    return str(contact.get("nickname") or "") if isinstance(contact, dict) else ""
...
def _share_card_from_json(raw: str) -> ShareCardInfo | None:
    ...
    return ShareCardInfo(
        ...
        is_wechat_miniprogram=_is_wechat_miniprogram(card),
    )
            ...
            if seg_type == "text":
                ...
                if card is None:
                    ...
                    if share_card is None:
                        ...
                        # Preserve each structural flag independently across all cards.
                        share_card = share_card.model_copy(
                            update={
                                "is_group_card": share_card.is_group_card or card.is_group_card,
                                "is_wechat_miniprogram": (
                                    share_card.is_wechat_miniprogram or card.is_wechat_miniprogram
                                ),
                            }
                        )
